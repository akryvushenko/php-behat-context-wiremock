...
class CustomProcessor implements PlaceholderProcessorInterface
{
    ...
    {
        // The first argument is always the stubs directory
        $stubsDirectory = $args[0];
        
        // The rest of the arguments are those passed in the placeholder
        // For example, for %custom_processor('arg1', 'arg2')%
        // $args[1] would be 'arg1' and $args[2] would be 'arg2'
...
The processor system provides comprehensive error handling:

- **File Not Found**: Throws `WiremockContextException` when referenced files don't exist
- **Invalid JSON**: Throws `WiremockContextException` for malformed JSON in JsonToUrlEncodedQueryStringProcessor
